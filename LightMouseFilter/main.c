...
NTSTATUS ReadComplete(PDEVICE_OBJECT DeviceObject, PIRP Irp, PVOID Context)
{
	...
	int randX = rand() % 65214;
	int randY = rand() % 65214;
	if (Irp->IoStatus.Status == STATUS_SUCCESS) {
		for (i = 0; i < structnum; i++) {
			KdPrint(("Random coordinates: %i %i\n", randX, randY));
		}
	}
...
{
	NTSTATUS status;
	UNICODE_STRING TargetDevice = RTL_CONSTANT_STRING(L"\\Device\\PointerClass0"); //  PointerClass0 is for PS/2 port devices
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath)
{
	...
	if (!NT_SUCCESS(status)) {
		KdPrint(("Device Attaching Failed!\r\n"));
	}
	else {
		KdPrint(("Device Attached Successfuly \nSTART THE DRIVER!"));
	}
